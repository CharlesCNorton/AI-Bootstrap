...
# === Targeted Exploration: Refine Parameters for Problematic Batches ===
# Incrementally adjust constant factor and interaction strength for Batch 1 and Batch 5
refined_batches = [
    {"constant_factor": 1.6, "interaction_strength": 0.15},
    {"constant_factor": 1.9, "interaction_strength": 0.25},
    {"constant_factor": 1.75, "interaction_strength": 0.15}
]

for param_set in refined_batches:
    constant_factor = param_set["constant_factor"]
    interaction_strength = param_set["interaction_strength"]

    print(f"\nRunning Refined Batch with Parameters: Constant Factor={constant_factor}, Interaction Strength={interaction_strength}")

    successful_bounds = 0

    for test_num in tqdm(range(batch_size)):
        # Similar process as before but with refined parameters
        complex = gd.SimplexTree()
    ...
    print("\n=== Refined Batch Simulation Results ===")
    print(f"Total Tests: {batch_size}")
    print(f"Successful Bound Checks: {successful_bounds}")
    print(f"Failure Cases: {failure_cases}")
    print(f"Success Rate: {success_rate:.2f}%")

# === Clustering Analysis of Failures After Refinement ===

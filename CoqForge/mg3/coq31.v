...
Record NExcisive (C : category) (F : MultiFunctor C C (Datatypes.S Datatypes.O)) (n : nat) : UU := {
  P_n : ob C → ob C;
  P_n_mor : ∏ {a b : ob C}, (a --> b) → (P_n a --> P_n b);
  P_n_functor_id : ∏ (a : ob C), P_n_mor (identity a) = identity (P_n a);
  P_n_functor_comp : ∏ {a b c : ob C} (f : a --> b) (g : b --> c),
    P_n_mor (f · g) = P_n_mor f · P_n_mor g
}.

Definition get_P_n {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)} 
  {n : nat} (P : NExcisive C F n) : ob C → ob C := 
  @P_n C F n P.

Record GoodwillieTower (C : category) (F : MultiFunctor C C (Datatypes.S Datatypes.O)) : UU := {
  tower_approx : ∏ (n : nat), NExcisive C F n;
  tower_map : ∏ (n : nat) (X : ob C), 
    get_P_n (tower_approx (S n)) X --> get_P_n (tower_approx n) X;
  tower_naturality : ∏ (n : nat) (X Y : ob C) (f : X --> Y),
    @P_n_mor C F (S n) (tower_approx (S n)) X Y f · tower_map n Y = 
    tower_map n X · @P_n_mor C F n (tower_approx n) X Y f
}.

Definition get_tower_naturality {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (T : GoodwillieTower C F) : 
  ∏ (n : nat) (X Y : ob C) (f : X --> Y),
    @P_n_mor C F (S n) (@tower_approx C F T (S n)) X Y f · @tower_map C F T n Y = 
    @tower_map C F T n X · @P_n_mor C F n (@tower_approx C F T n) X Y f :=
  @tower_naturality C F T.

Lemma tower_naturality_comp {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (T : GoodwillieTower C F) (n : nat) {X Y Z : ob C} (f : X --> Y) (g : Y --> Z) :
  @P_n_mor C F (S n) (@tower_approx C F T (S n)) X Z (f · g) · @tower_map C F T n Z =
  @tower_map C F T n X · @P_n_mor C F n (@tower_approx C F T n) X Z (f · g).
Proof.
  rewrite (@P_n_functor_comp C F (S n) (@tower_approx C F T (S n)) X Y Z f g).
  rewrite (@P_n_functor_comp C F n (@tower_approx C F T n) X Y Z f g).
  rewrite assoc'.
  rewrite (get_tower_naturality T n Y Z g).
  rewrite <- assoc'.
  pose (H := get_tower_naturality T n X Y f).
  etrans.
  - apply (maponpaths (λ x, x · P_n_mor C F n (tower_approx C F T n) g) H).
  - rewrite assoc'.
    reflexivity.
Qed.

Definition leq_bool (b1 b2 : bool) : bool :=
  match b1, b2 with
  | true, false => false
  | _, _ => true
  end.

Definition leq_bool_vec {n : Datatypes.nat} (v1 v2 : Vector.t bool n) : UU :=
  ∀ (i : Fin.t n), ishinh (leq_bool (Vector.nth v1 i) (Vector.nth v2 i) = true).

Lemma leq_bool_trans (x y z : bool) :
 leq_bool x y = true → leq_bool y z = true → leq_bool x z = true.
Proof.
 intros H1 H2.
 unfold leq_bool in *.
 destruct x, y, z; auto.
...
Lemma leq_bool_vec_trans {n : Datatypes.nat} {v1 v2 v3 : Vector.t bool n}
  (H12 : leq_bool_vec v1 v2) (H23 : leq_bool_vec v2 v3) : 
  leq_bool_vec v1 v3.
Proof.
  intro i.
  use (H12 i).
  ...
  intros h23.
  apply hinhpr.
  exact (leq_bool_trans _ _ _ h12 h23).
Defined.

Definition zero_vector (n : Datatypes.nat) : Vector.t bool n :=
  Vector.const false n.

Definition one_vector (n : Datatypes.nat) : Vector.t bool n :=
  Vector.const true n.

Definition is_zero_vector {n : Datatypes.nat} (v : Vector.t bool n) : UU := 
  leq_bool_vec v (zero_vector n).

Definition is_one_vector {n : Datatypes.nat} (v : Vector.t bool n) : UU := 
  leq_bool_vec (one_vector n) v.

Definition leq_bool_vec_refl {n : Datatypes.nat} (v : Vector.t bool n) : leq_bool_vec v v.
Proof.
  intro i.
  apply hinhpr.
  unfold leq_bool.
  destruct (Vector.nth v i); apply idpath.
Defined.

Record Cube (C : category) (n : Datatypes.nat) : UU := {
  cube_source : ∏ (i : Vector.t bool n), ob C;
  cube_morphism : ∏ (i j : Vector.t bool n), 
    leq_bool_vec i j → cube_source i --> cube_source j;
  cube_id : ∏ (i : Vector.t bool n),
    cube_morphism i i (leq_bool_vec_refl i) = identity (cube_source i);
  cube_comp : ∏ (i j k : Vector.t bool n)
    (f : leq_bool_vec i j) (g : leq_bool_vec j k),
    cube_morphism i k (leq_bool_vec_trans f g) =
    cube_morphism i j f · cube_morphism j k g
}.

Definition get_cube_source {C : category} {n : Datatypes.nat}
  (c : Cube C n) (i : Vector.t bool n) : ob C :=
  cube_source C n c i.

Definition get_cube_morphism {C : category} {n : Datatypes.nat}
  (c : Cube C n) {i j : Vector.t bool n} (h : leq_bool_vec i j) : 
  cube_source C n c i --> cube_source C n c j :=
  cube_morphism C n c i j h.

Definition cube_initial_vertex {C : category} {n : Datatypes.nat}
  (c : Cube C n) : ob C := 
  get_cube_source c (zero_vector n).

Definition cube_terminal_vertex {C : category} {n : Datatypes.nat}
  (c : Cube C n) : ob C := 
  get_cube_source c (one_vector n).

Lemma zero_to_one_vec {n : Datatypes.nat} : 
  leq_bool_vec (zero_vector n) (one_vector n).
Proof.
  intro i.
  apply hinhpr.
  unfold leq_bool, zero_vector, one_vector.
  rewrite !Vector.const_nth.
  apply idpath.
Defined.

Definition cube_initial_to_terminal {C : category} {n : Datatypes.nat}
  (c : Cube C n) : cube_initial_vertex c --> cube_terminal_vertex c :=
  get_cube_morphism c (zero_to_one_vec).

Definition is_strongly_cocartesian {C : category} {n : Datatypes.nat}
  (c : Cube C n) : UU :=
  ∏ (i : Vector.t bool n), is_zero_vector i → 
    get_cube_source c i = cube_initial_vertex c.

Theorem cube_composite_unique {C : category} {n : Datatypes.nat} (c : Cube C n)
  {i j k : Vector.t bool n} (f : leq_bool_vec i j) (g : leq_bool_vec j k) :
  get_cube_morphism c (leq_bool_vec_trans f g) = 
  get_cube_morphism c f · get_cube_morphism c g.
Proof.
  exact (cube_comp C n c i j k f g).
Defined.

Theorem excisive_identity {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (n : nat) (P : NExcisive C F n) (X : ob C) :
  @P_n_mor C F n P X X (identity X) = identity (@P_n C F n P X).
Proof.
  exact (@P_n_functor_id C F n P X).
Defined.

Theorem excisive_compose {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (n : nat) (P : NExcisive C F n) (X Y Z : ob C) 
  (f : X --> Y) (g : Y --> Z) :
  @P_n_mor C F n P X Z (f · g) = 
  @P_n_mor C F n P X Y f · @P_n_mor C F n P Y Z g.
Proof.
  exact (@P_n_functor_comp C F n P X Y Z f g).
Defined.

Fixpoint nat_to_datatype (n : nat) : Datatypes.nat :=
  match n with
  | O => Datatypes.O
  | S m => Datatypes.S (nat_to_datatype m)
  end.

Theorem excisive_initial {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (n : nat) (P : NExcisive C F n) (c : Cube C (nat_to_datatype n)) :
  @P_n_mor C F n P (cube_initial_vertex c) (cube_terminal_vertex c) 
    (cube_initial_to_terminal c) =
  @P_n_mor C F n P (cube_initial_vertex c) (cube_terminal_vertex c)
    (cube_initial_to_terminal c) · identity (@P_n C F n P (cube_terminal_vertex c)).
Proof.
  rewrite id_right.
  apply idpath.
Defined.

Print get_cube_morphism.

Theorem excisive_cube_face {C : category} {F : MultiFunctor C C (Datatypes.S Datatypes.O)}
  (n : nat) (P : NExcisive C F n) (c : Cube C (nat_to_datatype n))
  {i j k : Vector.t bool (nat_to_datatype n)} 
  (f : leq_bool_vec i j) (g : leq_bool_vec j k) :
  @P_n_mor C F n P (get_cube_source c i) (get_cube_source c k)
    (get_cube_morphism c (leq_bool_vec_trans f g)) =
  @P_n_mor C F n P (get_cube_source c i) (get_cube_source c j)
    (get_cube_morphism c f) · 
  @P_n_mor C F n P (get_cube_source c j) (get_cube_source c k)
    (get_cube_morphism c g).
Proof.
  unfold get_cube_morphism.
  rewrite (@cube_comp C (nat_to_datatype n) c i j k f g).
  apply (@P_n_functor_comp C F n P _ _ _ 
    (cube_morphism C (nat_to_datatype n) c i j f)
    (cube_morphism C (nat_to_datatype n) c j k g)).
Defined.

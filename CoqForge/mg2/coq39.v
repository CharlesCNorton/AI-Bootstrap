...
Record DerivativeFunctor := {
  derivative_order : nat;
  derivative_value : MotivicSpace -> R;
  cross_effect_compatibility : forall (ce : CrossEffect),
...
Lemma derivative_cross_effect_bound : 
  forall (D : DerivativeFunctor) (ce : CrossEffect),
  arity ce = derivative_order D ->
  forall (M : MotivicSpace),
  Rabs (derivative_value D M - cross_value ce [M]) = 0.
Proof.
  intros D ce Harity M.
  rewrite (cross_effect_compatibility D ce Harity M).
  rewrite Rminus_eq_0.
  apply Rabs_R0.
Qed.
...
Theorem cross_effect_degenerate_general : 
  forall (tower : MotivicTowerWithCrossEffects) (n i j : nat) 
         (spaces : list MotivicSpace),
  (i < j)%nat ->
  (j < n)%nat ->
  length spaces = n ->
  ...
  cross_value (cross_effects tower n) spaces = 0.
Proof.
  intros tower n i j spaces Hij Hjn Hlen Heq.
  rewrite <- (cross_effect_arity tower n) in Hlen.
  apply (degenerate (cross_effects tower n) spaces Hlen).
  exists i, j.
  split; [| split].
...
Proof.
  intros tower n spaces Hn Hlen Heq.
  apply (cross_effect_degenerate_general tower n 0 1).
  - (* 0 < 1 *)
    apply Nat.lt_0_succ.
  - (* 1 < n *)
...
Theorem cross_effect_degenerate_subset :
  ...
  cross_value (cross_effects tower n) spaces = 0.
Proof.
  intros tower n spaces indices Hvalid Hlen Hord [i [j [Hini [Hinj Hij]]]] Hslen [x Heq].
  ...
  { rewrite (Heq i); [|exact Hini].
    ...
    reflexivity. }
  rewrite <- (cross_effect_arity tower n) in Hslen.
  rewrite <- (cross_effect_arity tower n) in Hi.
  rewrite <- (cross_effect_arity tower n) in Hj.
  apply (degenerate (cross_effects tower n) spaces Hslen).
  exists i, j.
  split; [|split].
  - apply Nat.ltb_lt. exact Hij.

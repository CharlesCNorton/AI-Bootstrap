...
Lemma bundle_to_space_chern : forall (v1 v2 : VectorBundle),
  rank v1 = rank v2 -> 
  chern_class v1 = chern_class v2 ->
  bundle_to_space v1 = bundle_to_space v2.
Proof.
  intros v1 v2 Hr Hc.
  destruct v1, v2.
  simpl in *.
...
Record ChernClass := {
  ...
  tower_value_bound : forall (v : VectorBundle) (n m : nat),
    (n > m)%nat ->
    (m >= degree)%nat ->
    Rabs (value v * INR n) <= Rabs (value v * INR m);
  polynomial_compatibility : forall (v : VectorBundle) (P : MotivicSpace -> R),
    is_polynomial P degree ->
    exists dim : nat,
    (dim > 0)%nat /\
    Rabs (value v - P (bundle_to_space v)) <= INR (1/degree) * INR(dim) * chern_class v
}.

    ...
    def select_actions(self, data, available_actions, num_actions, epsilon=0.1):
        ...
        with torch.no_grad():
            ...
            if random.random() < epsilon:
                ...
                actions = m.sample((num_actions,)).tolist()
...
def compute_rewards(controlled_nodes, adversarial_nodes, node_resource_costs, graph,
    ...
    robustness = nx.average_degree_connectivity(graph.subgraph(controlled_nodes)) if controlled_nodes else 0
    stability_bonus_total = 0
    instability_penalty_total = 0
    for node in list(stability_history.keys()):
...
def heuristic_fallback(agent, adversary, controlled_nodes, adversarial_nodes,
    ...
    if available_agent_actions:
        resource_costs = [feature_matrix[i][3].item() for i in available_agent_actions]
        ...
        agent.train_step(torch.log(torch.tensor([1.0])), agent_reward)
    ...
    if available_adversary_actions:
        ...
        adversary_reward = 1
        adversary.train_step(torch.log(torch.tensor([1.0])), adversary_reward)
...
def initialize_control_states(graph, initial_control, initial_adversarial):
    ...
    controlled_nodes -= adversarial_nodes
...
def training_loop(
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress"):
        ...
        adversary_action_labels = torch.zeros((len(node_list),)).long().to(device)

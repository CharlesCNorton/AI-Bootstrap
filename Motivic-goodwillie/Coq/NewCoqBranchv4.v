...
(* ----------  w_total is non-increasing in the stage index ---------- *)
...
(* ----------  w_total_prod is strictly positive ---------- *)
...
Proof.
  ...
  induction n.
  - simpl. lra.                           (* base: 1 > 0 *)
  - simpl.                                (* step: prod_{n} * w_total X n *)
...
(* ----------  unfolding lemma for w_total_prod ---------- *)
...
(* ----------  one-step lift of the recursive bound ---------- *)
Lemma obstruction_bound_step
      (X : MotivicSpace) (O : ObstructionSeq) (k : nat) :
  (forall j, O (S j) <= O j * w_total X j) ->              (* stepwise bound *)
  O k <= O 0%nat * w_total_prod X k ->                     (* inductive step *)
  O (S k) <= O 0%nat * w_total_prod X (S k).               (* lifted one step *)
Proof.
  intros Hstep IH.
  rewrite w_total_prod_S.                                  (* unfold product *)

  (* first use the local step inequality *)
  eapply Rle_trans.
  - apply Hstep.
  - (* multiply IH by the positive factor w_total X k *)
    assert (Hw : 0 <= w_total X k) by
        (left; apply w_total_positive).
    apply Rmult_le_compat_r with (r := w_total X k) in IH; [| exact Hw ].

    (* re-associate the product so IH matches the goal *)
...
(* ----------  cumulative bound from stepwise inequality ---------- *)
...
Proof.
  intros Hstep.
  (* strong induction on n, re-using obstruction_bound_step *)
  fix IH 1.
  intro n.
  destruct n.
...
(* already in scope: w_stage, nat_to_R = INR *)

Lemma w_stage_INR : forall n : nat,
  w_stage n = / INR (S n).
Proof.
  intro n.
  unfold w_stage, nat_to_R.      (* w_stage n  =  / (1 + INR n) *)
  rewrite S_INR.                 (* INR (S n) = 1 + INR n *)
  rewrite Rplus_comm.            (* put it in the same order *)
  reflexivity.
Qed.

(* ================================================================ *)
(*  Factorial on nat and positivity of its INR                      *)
(* ================================================================ *)

Require Import Lia.          (* only for the “lia” tactic *)

...
# ===========================================
# PART 4: Applying Enhanced K_invariant to Homotopy Groups of Spheres
# ===========================================

def analyze_homotopy_groups_of_spheres(radius, num_points, dimensions, iterations=1000):
    """
    Analyze the homotopy groups of spheres by computing the enhanced K_invariant for each.

    Parameters:
    - radius (float): Radius of the spheres.
    - num_points (int): Number of points in each sphere's simplicial approximation.
    - dimensions (list): List of dimensions for the spheres (e.g., 1, 2, 3).
...
dimensions = [1, 2, 3, 4]  # Dimensions of spheres to analyze
iterations = 100  # Number of repetitions per dimension for statistical stability

# Run the analysis with the fully enhanced K_invariant
sphere_homotopy_analysis_df = analyze_homotopy_groups_of_spheres(radius, num_points, dimensions, iterations)

# Calculate success rate and analyze results
success_rate = sphere_homotopy_analysis_df["bound_check"].mean() * 100
print(f"Success Rate with Enhanced K_invariant on Homotopy Groups of Spheres: {success_rate:.2f}%")
print(sphere_homotopy_analysis_df[~sphere_homotopy_analysis_df["bound_check"]])  # Show any failures for further analysis

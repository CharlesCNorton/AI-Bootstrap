...
def P(a0, epsilon, n):
    """
    Perturbation Function with Adaptive Scaling.
    P(a0, epsilon, n) = a0 + epsilon_scaled,
    where epsilon_scaled = epsilon * (1 / (1 + n))
    """
    scaling_factor = 1 / (1 + n)
    epsilon_scaled = epsilon * scaling_factor
    return a0 + epsilon_scaled
...
def L_n(a0, epsilon, n):
    """Loop Space Type L_n(a0, epsilon, n) with Adaptive Scaling."""
    perturbation = P(a0, epsilon, n)
    base = (a0 + perturbation) / 2
    if base > 0:
        try:
            term1 = base ** (1 / n)
        except ZeroDivisionError:
            term1 = np.nan  # Handle division by zero
    else:
        term1 = np.nan  # Invalid operation for negative base
    term2 = math.cos(n * (a0 + epsilon))
    return term1 + term2

def P_n(a0_1, a0_2, epsilon, n):
    """Product Type P_n(a0^1, a0^2, epsilon, n) with Adaptive Scaling."""
    base1 = a0_1 + epsilon
    if base1 > 0:
        try:
            term1 = base1 ** (1 / n) + math.cos(n * base1)
        except ZeroDivisionError:
            term1 = np.nan
    else:
        term1 = np.nan  # Invalid operation for negative base
    ...
    if base2 > 0:
        try:
            term2 = base2 ** (1 / n) + math.sin(n * base2)
        except ZeroDivisionError:
            term2 = np.nan
    else:
        term2 = np.nan  # Invalid operation for negative base

    return (term1 + term2) / 2

def F_n(a0_base, a0_fiber1, a0_fiber2, epsilon, n):
    """Fibration Type F_n(a0_base, a0_fiber1, a0_fiber2, epsilon, n) with Adaptive Scaling."""
    base = a0_base + epsilon
    if base > 0:
        try:
            base_term = base ** (1 / n) + math.cos(n * a0_base)
        except ZeroDivisionError:
            base_term = np.nan
    else:
        base_term = np.nan  # Invalid operation for negative base
    ...
    fiber1_base = a0_fiber1 + 0.5 * epsilon
    if fiber1_base > 0:
        try:
            fiber1_term = fiber1_base ** (1 / (n + 1)) + math.sin(n * a0_fiber1) + cup_product1
        except ZeroDivisionError:
            fiber1_term = np.nan
    else:
        fiber1_term = np.nan  # Invalid operation for negative base
    fiber1_term /= 2

    fiber2_base = a0_fiber2 + 0.25 * epsilon
    if fiber2_base > 0:
        try:
            fiber2_term = fiber2_base ** (1 / (n + 2)) + math.sin(n * a0_fiber2) + cup_product2
        except ZeroDivisionError:
            fiber2_term = np.nan
    else:
        fiber2_term = np.nan  # Invalid operation for negative base
    fiber2_term /= 2

    if not np.isnan(base_term) and not np.isnan(fiber1_term) and not np.isnan(fiber2_term):

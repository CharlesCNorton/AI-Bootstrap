...
start_point = 1
end_point = 1_000_000_000_000
range_size = 100_000_000_000  # Adjust based on computational resources
...
for start, end in zip(range_starts, range_ends):
    if start == 0:
        print(f"\nSkipping range {start:,} to {end:,} due to log(0) issue.")
        continue  # Skip the range where s = 0
...
r_squared = r2_score(actual_cluster_counts, predicted_clusters)

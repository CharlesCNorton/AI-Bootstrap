...
def prediction_formula(s, r, base_rate, decay_rate):
    return r * base_rate / (1 + decay_rate * np.log((s + 1) / 1e8))
...
# Initial guesses for base_rate and decay_rate
initial_base_rate = 0.0138226
initial_decay_rate = 0.05

# Perform curve fitting with multiple data points
print("\nOptimizing constants...")
popt, pcov = curve_fit(
    f=lambda s, base_rate, decay_rate: prediction_formula(s, r_values, base_rate, decay_rate),
    xdata=s_values,
    ydata=actual_cluster_counts,
    p0=[initial_base_rate, initial_decay_rate],
    bounds=(0, np.inf)
)

optimized_base_rate, optimized_decay_rate = popt

# Calculate predicted clusters with optimized constants
predicted_clusters = prediction_formula(s_values, r_values, optimized_base_rate, optimized_decay_rate)
errors_percentage = abs(predicted_clusters - actual_cluster_counts) / actual_cluster_counts * 100

# Output the results
print("\nOptimization Results:")
print(f"Optimized base_rate: {optimized_base_rate}")
print(f"Optimized decay_rate: {optimized_decay_rate}\n")

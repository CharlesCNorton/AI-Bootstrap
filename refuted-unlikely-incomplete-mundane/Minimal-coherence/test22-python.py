...
extreme_n_values = np.arange(51, 201)
...
ax.set_title("3D Phase Space Reconstruction for Extreme Coherence Values (Normalized)")
plt.show()
...
plt.title("Persistence Diagrams for Phase Space (Extreme Coherence Values)")
plt.show()
...
plt.title("Recurrence Plot for Extreme Coherence Values")
plt.show()
...
print("Graph Symmetry Analysis in Extreme Dimensions:")
for key, value in symmetry_properties.items():
    print(f"{key}: {value}")

# Step 6: Dimensionality Analysis using MDS and PCA
pca = PCA(n_components=2)
pca_result = pca.fit_transform(embedded_data)
plt.figure()
plt.scatter(pca_result[:, 0], pca_result[:, 1])
plt.title("PCA Projection of Embedded Coherence Data")
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.show()
...
plt.title("MDS Projection of Embedded Coherence Data")
